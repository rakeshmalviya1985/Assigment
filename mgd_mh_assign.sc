/* HP Port 11517 20100115 - From pendingcheckin/temp/mgd_mh_assign.sc 1.8 */
#pragma ident "@(#)mgd_mh_assign.sc	1.11 09/25/12 EDS"
/********************************************************************************/
/*                                                                              */
/*   PROGRAM:     mgd_mh_assign                                                 */
/*                                                                              */
/*   DESCRIPTION: This program checks the PIHP and PAHP eligibility and creates */
/*                assignments for the beneficiaries in potential PIHP/PAHP table*/
/*                The potential PIHP/PAHP table t_mc_re_pihp_pahp is created by */
...
/* ---------- ------  -----------------  -----------------                      */
/* 05/01/2007 10218   Srinivas Donepudi  Initial release.                       */
...
EXEC SQL BEGIN DECLARE SECTION;
    ...
    static       char DEFAULT_PIHP_ASSIGN[2+1]  = "77";  /* Default - PIHP Assignment */
    static       char DEFAULT_PAHP_ASSIGN[2+1]  = "78";  /* Default - PAHP Assignment */
    static const char PIHP_ELIG_CHG_REASON[2+1] = "79";  /* PIHP elig criteria change */
    static const char PAHP_ELIG_CHG_REASON[2+1] = "80";  /* PAHP elig criteria change */
    ...
    static int    POLICY_START_DATE = 20070701;
...
/*  static int    mc_assn_end_dte = 0;         * managed care assignment end date   */

    static int    dte_enroll_pihp;
    static int    dte_enroll_pahp;

    static int    sak_pub_hlth_pihp;
    static int    sak_pub_hlth_pahp;
    static int    sak_pmp_svc_pihp_cap;
    static int    sak_pmp_svc_pahp_cap;
    static int    sak_pmp_svc_pihp_nocap;
    static int    sak_pmp_svc_pahp_nocap;

    static int     autoassign_pihp = mgd_FALSE;
    static int     autoassign_pahp = mgd_FALSE;
...
static  int    cnt_pihp_assign     = 0;
static  int    cnt_pahp_assign     = 0;
static  int    cnt_pihp_extend     = 0;
static  int    cnt_pahp_extend     = 0;
static  int    cnt_pihp_report     = 0;
static  int    cnt_pahp_report     = 0;
static  int    cnt_pihp_notelig    = 0;
static  int    cnt_pahp_notelig    = 0;
static  int    cnt_pihp_continued  = 0;
static  int    cnt_pahp_continued  = 0;
static  int    date_hwend          = 0;
static  int    date_kcint          = 0;
static  int    SAK_ENT_PIHP_PAHP_AUTOASSIGN = 0;
...
static  int     enrolled_pihp  = mgd_FALSE;
static  int     enrolled_pahp  = mgd_FALSE;
static  int     noncap_svc_loc = mgd_FALSE;
static  int     eligible_for_pihp = mgd_FALSE;
static  int     eligible_for_pahp = mgd_FALSE;
...
AND    pgm.cde_pgm_health in ('TXIX','MKN','MN','P19')
...
/* DESCRIPTION: Get all MH assignments (PIHP/PAHP) for a beneficiary */
...
AND elig.cde_status1 <> 'H'
AND elig.sak_pub_hlth IN (:sak_pub_hlth_pihp,
                          :sak_pub_hlth_pahp)
...
static bool is_recip_newly_eligible(void);
...
static int make_pmp_assgn(int parm_pmp_svc_loc, char *parm_assgn_rsn, int parm_ent_autoassign, int parm_assgn_dte,
                          int parm_assgn_end_dte, int parm_sak_pub_hlth, int parm_sak_prov_mbr, char *parm_cde_svc_loc_mbr );
...
/* ---------- ----- ----------------- --------------------------------      */
/* 05/01/2007 10218 Srinivas Donepudi Initial Creation                      */
/****************************************************************************/
int main(int argc, char *argv[])
{
    ...
    while (cursor_rc == ANSI_SQL_OK)
    {
        ...
        enrolled_pihp     = mgd_FALSE;
        enrolled_pahp     = mgd_FALSE;
        eligible_for_pihp = mgd_FALSE;
        eligible_for_pihp = mgd_FALSE;
            ...
            (continue_process == mgd_TRUE))
        {
           ...
           if (enrolled_pihp == mgd_FALSE)
           {
               rc_elig = check_pgm_elig(sak_pub_hlth_pihp,benefit_mth_start);
               if ( rc_elig == mgd_TRUE )
                   eligible_for_pihp = mgd_TRUE;
               else
                   eligible_for_pihp = mgd_FALSE;

               if ((eligible_for_pihp) && (autoassign_pihp))
                  enrolled_pihp = do_default_assign(benefit_mth_start, EOT, sak_pub_hlth_pihp);
               else
                  cnt_pihp_notelig++;
           }
           else
           ...
           /* when no current assign exists or the current assign was ended since it is not valid */
           if (enrolled_pahp == mgd_FALSE)
           {
               rc_elig = check_pgm_elig(sak_pub_hlth_pahp,benefit_mth_start);
               if ( rc_elig == mgd_TRUE )
                   eligible_for_pahp = mgd_TRUE;
               else
                   eligible_for_pahp = mgd_FALSE;

               if ((eligible_for_pahp) && (autoassign_pahp))
                  enrolled_pahp = do_default_assign(benefit_mth_start, EOT, sak_pub_hlth_pahp);
               else
                  cnt_pahp_notelig++;
           }
           else
        ...
        else /* Daily records - Only virgin eligibles/SED LOC benes will be processed */
        {
            ...
            if (((is_recip_newly_eligible() == mgd_TRUE) ||
                 (c_is_valid_sed_level_of_care(&mc_is_sed_loc, &rtn_msg_str,
                                        sql_pntl_rcd.sak_recip, benefit_mth_start, mgd_FALSE) == mgd_TRUE) ) &&
                (continue_process == mgd_TRUE))
            {
               rc_elig = check_pgm_elig(sak_pub_hlth_pihp,benefit_mth_start);
               if ( rc_elig == mgd_TRUE )
               ...
               if (eligible_for_pihp)
               {
                  check_cur_assign(sak_pub_hlth_pihp, benefit_mth_start);

                  if ((!enrolled_pihp) && (autoassign_pihp))
                     enrolled_pihp = do_default_assign(benefit_mth_start,
               ...
               if (eligible_for_pahp)
               {
                  check_cur_assign(sak_pub_hlth_pahp, benefit_mth_start);

                  if ((!enrolled_pahp) && (autoassign_pahp))
                     enrolled_pahp = do_default_assign(benefit_mth_start,
                                                       ((benefit_mth_start == last_kaecses_elig_dte)?EOT:benefit_mth_end),
                                                       sak_pub_hlth_pahp);
                  else
                  {
...
static void display_rcd_cnt(void)
{
        ...
        ((cnt_potential_read > 0) && (cnt_potential_read % 500) == 0))
    {
        ...
        fprintf(stderr, "\t\t# of PIHP Assignments Created ................. [%9d]\n", cnt_pihp_assign);
        fprintf(stderr, "\t\t# of PAHP Assignments Created ................. [%9d]\n", cnt_pahp_assign);
        fprintf(stderr, "\t\t# of PIHP Assignments Extended ................ [%9d]\n", cnt_pihp_extend);
        fprintf(stderr, "\t\t# of PAHP Assignments Extended ................ [%9d]\n", cnt_pahp_extend);
        fprintf(stderr, "\t\t# of PIHP Assignments Reported ................ [%9d]\n", cnt_pihp_report);
        fprintf(stderr, "\t\t# of PAHP Assignments Reported ................ [%9d]\n", cnt_pahp_report);
        fprintf(stderr, "\t\t# of PIHP Assignments Not Eligible ............ [%9d]\n", cnt_pihp_notelig);
        fprintf(stderr, "\t\t# of PAHP Assignments Not Eligible ............ [%9d]\n", cnt_pahp_notelig);
        fprintf(stderr, "\t\t# of PIHP Assignments Continued ............... [%9d]\n", cnt_pihp_continued);
        fprintf(stderr, "\t\t# of PAHP Assignments Continued ............... [%9d]\n", cnt_pahp_continued);
        fflush(stderr);
    }
}
/*********************************************************************/
...
static void initialize(int argnum, char *argmnts[])
{
    ...
    rc = c_get_mc_entity(&entity_str, &rtn_msg_str, "APP");
    check_rc(&rtn_msg_str, rc, FNC);
    SAK_ENT_PIHP_PAHP_AUTOASSIGN = entity_str.sak_mc_entity;

    if (arg_run_level == MONTHLY)
    {
        if (!c_is_on_same_month(dte_enroll_pihp, first_of_next_month))
        {
            fprintf(stderr, "\n\nPIHP Enrollment Date [%d] IS NOT CORRECT!!!  "
                            "It should be set to [%d]!!!  T_SYSTEM_PARMS MAY NEED TO BE UPDATED.\n\n\n",
                dte_enroll_pihp, first_of_next_month);
            exit(FAILURE);
        }

        if (!c_is_on_same_month(dte_enroll_pahp, first_of_next_month))
        {
            fprintf(stderr, "\n\nPAHP Enrollment Date [%d] IS NOT CORRECT!!!  "
                            "It should be set to [%d]!!!  T_SYSTEM_PARMS MAY NEED TO BE UPDATED.\n\n\n",
                dte_enroll_pahp, first_of_next_month);
            exit(FAILURE);
        }
    }
    ...
    fprintf(stderr, "\nmc_sak_elig_stop = [ %d ]\n", mc_sak_elig_stop);
    fprintf(stderr, "SAK_ENT_PIHP_PAHP_AUTOASSIGN = [ %d ]\n", SAK_ENT_PIHP_PAHP_AUTOASSIGN);
...
static int det_pgm_assign_dte(int parm_sak_pub_hlth)
{
    if (parm_sak_pub_hlth == sak_pub_hlth_pihp)
    {
        strcpy(recip_cde_pgm_health, "PIHP");
        /* return(dte_enroll_pihp); */
        return 0;
    }
    else if (parm_sak_pub_hlth == sak_pub_hlth_pahp)
    {
        strcpy(recip_cde_pgm_health, "PAHP");
        /* return(dte_enroll_pahp); */
        return 0;
    }
    else
    {
...
static void finalize(void)
{
    ...
    fprintf(err_rpt_file_ptr, "\t# of PIHP Assignments Created ................. [%9d]\n", cnt_pihp_assign);
    fprintf(err_rpt_file_ptr, "\t# of PAHP Assignments Created ................. [%9d]\n", cnt_pahp_assign);
    fprintf(err_rpt_file_ptr, "\t# of PIHP Assignments Extended ................ [%9d]\n", cnt_pihp_extend);
    fprintf(err_rpt_file_ptr, "\t# of PAHP Assignments Extended ................ [%9d]\n", cnt_pahp_extend);
    fprintf(err_rpt_file_ptr, "\t# of PIHP Assignments Reported ................ [%9d]\n", cnt_pihp_report);
    fprintf(err_rpt_file_ptr, "\t# of PAHP Assignments Reported ................ [%9d]\n", cnt_pahp_report);
    fprintf(err_rpt_file_ptr, "\t# of PIHP Assignments Not Eligible ............ [%9d]\n", cnt_pihp_notelig);
    fprintf(err_rpt_file_ptr, "\t# of PAHP Assignments Not Eligible ............ [%9d]\n", cnt_pahp_notelig);
    fprintf(err_rpt_file_ptr, "\t# of PIHP Assignments Continued ............... [%9d]\n", cnt_pihp_continued);
    fprintf(err_rpt_file_ptr, "\t# of PAHP Assignments Continued ............... [%9d]\n", cnt_pahp_continued);
...
static void get_assign_dates(void)
{
    ...
    EXEC SQL SELECT to_number(to_char(last_day(add_months(SYSDATE, -DATE_PARM_1)) +1, 'YYYYMMDD')),
               ...
               INTO :mc_retro_date,
                    :dte_current
               FROM t_system_parms
              WHERE nam_program = 'MGDMHRET';

    if ( sqlca.sqlcode != 0 )
    {
        fprintf(stderr, "Failed to retrieve retro date using parm MGDMHRET. \n");
        sqlerror();
        exit(-1);
    }
    ...
    if (rc == FUNCTION_SUCCESSFUL)
    {
/*      last_mthly_kaecses_dte = sql_date_parm_1;  */
        last_kaecses_elig_dte  = sql_dte_parm_2;
    }
    else
        exit(-1);

     /* managed care assignment end date. 14303, never used. * /
...
*/
    ...
    /********************** PIHP Program Data ************************* */
    rc = c_get_pub_hlth_pgm(&hlth_pgm_str, &rtn_msg_str, 0, "PIHP");
    check_rc(&rtn_msg_str, rc, FNC);
    sak_pub_hlth_pihp = hlth_pgm_str.sak_pub_hlth;

    rc = c_get_dte_enroll(&enroll_dte_str, &rtn_msg_str, sak_pub_hlth_pihp);
    check_rc(&rtn_msg_str, rc, FNC);
    dte_enroll_pihp = enroll_dte_str.dte;

    /* Determine if Autoassignment is 'on' for the PIHP program... */
    autoassign_pihp = is_pgm_autoassignable(sak_pub_hlth_pihp);

    /* Determine the sak-pmp_svc_loc for PIHP with cap */
    strncpy(ind_focus,"Y",1);
    sak_pmp_svc_pihp_cap = get_pmp_sak(sak_pub_hlth_pihp,dte_current,ind_focus);

    /* Determine the sak-pmp_svc_loc for PIHP with no cap */
    strncpy(ind_focus,"N",1);
    sak_pmp_svc_pihp_nocap = get_pmp_sak(sak_pub_hlth_pihp,dte_current,ind_focus);


    /* ********************* PAHP Program Data ************************* */
    rc = c_get_pub_hlth_pgm(&hlth_pgm_str, &rtn_msg_str, 0, "PAHP");
    check_rc(&rtn_msg_str, rc, FNC);
    sak_pub_hlth_pahp = hlth_pgm_str.sak_pub_hlth;

    rc = c_get_dte_enroll(&enroll_dte_str, &rtn_msg_str, sak_pub_hlth_pahp);
    check_rc(&rtn_msg_str, rc, FNC);
    dte_enroll_pahp = enroll_dte_str.dte;

    /* Determine if Autoassignment is 'on' for the PAHP program... */
    autoassign_pahp = is_pgm_autoassignable(sak_pub_hlth_pahp);

    /* Determine the sak-pmp_svc_loc for PAHP with cap */
    strncpy(ind_focus,"Y",1);
    sak_pmp_svc_pahp_cap = get_pmp_sak(sak_pub_hlth_pahp,dte_current,ind_focus);

    /* Determine the sak-pmp_svc_loc for PAHP with no cap */
    strncpy(ind_focus,"N",1);
    sak_pmp_svc_pahp_nocap = get_pmp_sak(sak_pub_hlth_pahp,dte_current,ind_focus);
    ...
    fprintf(err_rpt_file_ptr, "*************************************************************\n");
    fprintf(err_rpt_file_ptr, " PIHP/PAHP Autoassignment Log for Date [%d] 24 hr Time [%s]\n", dte_current, curr_time);
    fprintf(err_rpt_file_ptr, "*************************************************************\n");

    fprintf(stderr, "PIHP normal enrollment date   = [ %d ]  MGDENRPI \n", dte_enroll_pihp);
    fprintf(stderr, "PAHP normal enrollment date   = [ %d ]  MGDENRPA \n", dte_enroll_pahp);
    fprintf(stderr, "Kancare initial date          = [ %d ]  MGDKCINT \n", date_kcint);       /* CO14303 */
    fprintf(stderr, "Kancare HW End  date          = [ %d ]  MGDHWEND \n", date_hwend);

    fprintf(err_rpt_file_ptr, "PIHP normal enrollment date  = [ %d ]\n", dte_enroll_pihp);
    fprintf(err_rpt_file_ptr, "PAHP normal enrollment date  = [ %d ]\n", dte_enroll_pahp);
    fprintf(err_rpt_file_ptr, "Kancare initial date         = [ %d ]  MGDKCINT \n", date_kcint);  /* CO14303 */
    fprintf(err_rpt_file_ptr, "Kancare HW End  date         = [ %d ]  MGDHWEND \n", date_hwend);

    fprintf(stderr, "sak_pub_hlth_pihp  = [ %d ]\n", sak_pub_hlth_pihp);
    fprintf(stderr, "sak_pub_hlth_pahp  = [ %d ]\n", sak_pub_hlth_pahp);

    fprintf(stderr, "sak_pmp_svc_pihp_cap   = [ %d ]\n", sak_pmp_svc_pihp_cap);
    fprintf(stderr, "sak_pmp_svc_pihp_nocap = [ %d ]\n", sak_pmp_svc_pihp_nocap);
    fprintf(stderr, "sak_pmp_svc_pahp_cap   = [ %d ]\n", sak_pmp_svc_pahp_cap);
    fprintf(stderr, "sak_pmp_svc_pahp_nocap = [ %d ]\n", sak_pmp_svc_pahp_nocap);

    fprintf(stderr, "autoassign_pihp = [ %d ]\n", autoassign_pihp);
    fprintf(stderr, "autoassign_pahp  = [ %d ]\n", autoassign_pahp);
...
static int is_pgm_autoassignable(int parm_sak_pub_hlth)
{
    ...
    EXEC SQL SELECT ind_mc_autoassign
              ...
              WHERE c.sak_prov_pgm = e.sak_prov_pgm AND
                    e.cde_prov_pgm IN ('PIHP', 'PAHP') AND
...
static int do_default_assign(int parm_date, int parm_end_date, int parm_sak_pub_hlth)
{
   ...
   {
     if (parm_sak_pub_hlth == sak_pub_hlth_pihp)
        parm_sak_pmp_svc_loc = sak_pmp_svc_pihp_nocap;
     else
        parm_sak_pmp_svc_loc = sak_pmp_svc_pahp_nocap;
   }
   else
   {
     if (parm_sak_pub_hlth == sak_pub_hlth_pihp)
        parm_sak_pmp_svc_loc = sak_pmp_svc_pihp_cap;
     else
        parm_sak_pmp_svc_loc = sak_pmp_svc_pahp_cap;
   }
                                                            /* CO14303 */
     if (aim_cycle_date >= date_kcint)
     {
        ...
        {
           fprintf(err_rpt_file_ptr, "\t\tDate is after the program end date. sak_recip [%d] , BID [%s], pgm [%s] \n",
   ...
   make_pmp_assgn(parm_sak_pmp_svc_loc,
                 (parm_sak_pub_hlth == sak_pub_hlth_pihp)?DEFAULT_PIHP_ASSIGN:DEFAULT_PAHP_ASSIGN,
                  SAK_ENT_PIHP_PAHP_AUTOASSIGN,
...
static void check_cur_assign(int parm_sak_pub_hlth, int parm_date)
{
   ...
   if (rc == mgd_TRUE)
   {
      ...
      if (noncap_svc_loc == mgd_TRUE)
      {
        if (parm_sak_pub_hlth == sak_pub_hlth_pihp)
        {
           if (assign_info_str.sak_pmp_ser_loc != sak_pmp_svc_pihp_nocap)
               write_rpt_tbl(CHG_SVCLOC_TO_NONCAP);

           enrolled_pihp = mgd_TRUE;
        }
        else
        {
           if (assign_info_str.sak_pmp_ser_loc != sak_pmp_svc_pahp_nocap)
               write_rpt_tbl(CHG_SVCLOC_TO_NONCAP);

           enrolled_pahp = mgd_TRUE;
        }
      }
      else
      {
        if (parm_sak_pub_hlth == sak_pub_hlth_pihp)
        {
           if (assign_info_str.sak_pmp_ser_loc != sak_pmp_svc_pihp_cap)
        ...
        {
           if (assign_info_str.sak_pmp_ser_loc != sak_pmp_svc_pahp_cap)
               write_rpt_tbl(CHG_SVCLOC_TO_CAP);
...
/****************************************************************************/
/*                                                                          */
/* Function:    is_recip_newly_eligible()                                   */
/*                                                                          */
/* Description: This function determines if the beneficiary is newly        */
/*              eligible                                                    */
/*                                                                          */
/* Date       CO    SE                 Description                          */
/* ---------- ----- ----------------- --------------------------------------*/
/* 04/30/2006 10218 Srinivas Donepudi Initial Creation                      */
/*                                                                          */
/****************************************************************************/
static bool is_recip_newly_eligible(void)
{
    int rc = mgd_FALSE;
    char FNC[] = "is_recip_newly_eligible()";

    EXEC SQL BEGIN DECLARE SECTION;
        int sql_count = 0;
        int sql_active_count = 0;
        int sql_idcard_count = 0;
    EXEC SQL END DECLARE SECTION;

    if (arg_rsn_rpting == mgd_TRUE)
    {
        fprintf(err_rpt_file_ptr, "\tIs the bene a virgin eligible beneficiary ...  ");
    }

    EXEC SQL
         SELECT count(*)
           INTO :sql_count
           FROM t_re_elig a, t_pub_hlth_pgm b
          WHERE a.sak_pub_hlth = b.sak_pub_hlth
            AND b.cde_type in ('MJ','DU','SA','BO')
            AND a.dte_effective <= :benefit_mth_end
            AND a.dte_end >= :benefit_mth_start
            AND a.sak_recip = :sql_pntl_rcd.sak_recip
            AND a.dte_created < :aim_cycle_date;

    if (sqlca.sqlcode != ANSI_SQL_OK)
    {
        fprintf (stderr, "%s - ERROR checking virgin eligibility of the recip [%d]\n",FNC, sql_pntl_rcd.sak_recip);
        sqlerror();
        ind_abend = mgd_TRUE;
        finalize();
        exit(FAILURE);
    ...
    if (sql_count <= 0)
    {
        ...
        if (arg_rsn_rpting == mgd_TRUE)
        {
           fprintf(err_rpt_file_ptr, "YES - bene was not eligible previously.\n");
        }
    }
    else
    {
       /* The following sql is needed to catch the situation when current month eligibility
          is extended to future month since the last updated date is the only updated when this
          happens and the above sql will not catch this */
        EXEC SQL
         SELECT count(*)
           INTO :sql_active_count
           FROM t_re_elig a, t_pub_hlth_pgm b
          WHERE a.sak_pub_hlth = b.sak_pub_hlth
            AND b.cde_type in ('MJ','DU','SA','BO')
            AND a.dte_effective <= :benefit_mth_end
            ...
            AND a.cde_status1 = ' '
            AND a.dte_created      < :aim_cycle_date
            AND a.dte_last_updated >= :aim_cycle_date;

        if (sqlca.sqlcode != ANSI_SQL_OK)
        {
            fprintf (stderr, "%s - ERROR checking virgin elig for the recip [%d]\n",FNC, sql_pntl_rcd.sak_recip);
            sqlerror();
            ind_abend = mgd_TRUE;
            finalize();
            exit(FAILURE);
        ...
        if (sql_count == sql_active_count)
        {
          /* An extra check to make sure that Id card was issued previously */
          EXEC SQL
           SELECT count(*)
             INTO :sql_idcard_count
             FROM t_re_id_card
            WHERE sak_recip = :sql_pntl_rcd.sak_recip
              AND dte_coverage_begin <= :benefit_mth_end
              AND dte_coverage_end   >= :benefit_mth_start;

          if (sqlca.sqlcode != ANSI_SQL_OK)
          {
            fprintf (stderr, "%s - ERROR checking idcard issue check for the recip [%d]\n",FNC, sql_pntl_rcd.sak_recip);
            sqlerror();
            ind_abend = mgd_TRUE;
            finalize();
            exit(FAILURE);
          ...
          else
          {
             ...
             if (arg_rsn_rpting == mgd_TRUE)
             {
               fprintf(err_rpt_file_ptr, "NO - bene was eligible previously.\n");
             }
          }
        }
        else
    ...
    while ( fetch_assign() == ANSI_SQL_OK )
    {
      if (arg_rsn_rpting == mgd_TRUE)
      {
        ...
        if (eligible)
        {
           ...
           {
             if (pmp_assign_info.sak_pub_hlth == sak_pub_hlth_pihp)
             {
               if (sak_pmp_svc_pihp_nocap == pmp_assign_info.sak_pmp_ser_loc)
               {
                 pmpmatch = mgd_TRUE;
               }
               else
             ...
             {
               if (sak_pmp_svc_pahp_nocap == pmp_assign_info.sak_pmp_ser_loc)
               {
                  pmpmatch = mgd_TRUE;
               }
               else
           ...
           else
           {
             if (pmp_assign_info.sak_pub_hlth == sak_pub_hlth_pihp)
             {
              if (sak_pmp_svc_pihp_cap == pmp_assign_info.sak_pmp_ser_loc)
                 pmpmatch = mgd_TRUE;
              else
                 pmpmatch = mgd_FALSE;
             }
             ...
             {
               if (sak_pmp_svc_pahp_cap == pmp_assign_info.sak_pmp_ser_loc)
                  pmpmatch = mgd_TRUE;
               else
                  pmpmatch = mgd_FALSE;
             }
        ...
        if ((eligible) && (pmpmatch))
        {
           ...
           if (pmp_assign_info.sak_pub_hlth == sak_pub_hlth_pihp)
               enrolled_pihp = mgd_TRUE;
           else if (pmp_assign_info.sak_pub_hlth == sak_pub_hlth_pahp)
               enrolled_pahp = mgd_TRUE;
           else {
        ...
        if (!eligible || !pmpmatch)
        {
           ...
           {
              if (!pmpmatch)
                 elig_end_rsn = (char*)PIHP_ELIG_CHG_REASON;
              else
           ...
           {
              if (!pmpmatch)
                 elig_end_rsn = (char*)PAHP_ELIG_CHG_REASON;
              else
                 elig_end_rsn = (char*)MC_ELIG_END_REASON;
           }
...
static void write_rpt_tbl(int cde_rsn)
{
    ...
    if ( assign_info_str.sak_pub_hlth == sak_pub_hlth_pihp )
         cnt_pihp_report++;
    else if ( assign_info_str.sak_pub_hlth == sak_pub_hlth_pahp )
         cnt_pahp_report++;
